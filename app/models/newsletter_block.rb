...
# claim. The ESP's merge tag is inserted verbatim into the claim URL and
...
class NewsletterBlock
  ...
  def initialize(publication, daily_call: nil)
    ...
    @daily_call = daily_call || publication.todays_call
  end

  def claim_url(email_value = publication.email_merge_tag)
    "#{DailyBingo.public_host}/c/#{publication.public_code}/today?email=#{email_value}"
  end
